...
library(fastDummies)
final_data <- dummy_cols(final_data, select_columns = "state", remove_first_dummy = TRUE, remove_selected_columns = TRUE)
...
# 删除 customers_day，保留其他所有变量作为特征
X_train <- model.matrix(~ . - customers_day, data = train_data)
X_test <- model.matrix(~ . - customers_day, data = test_data)

y_train <- train_data$customers_day
y_test <- test_data$customers_day
