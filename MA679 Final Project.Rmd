...
columns_to_remove <- c( # List of columns to remove
  ...
  "total_customers",    # Same information like customers_day
  "FLOOD_CAUSE",
  "MAGNITUDE",
  "MAGNITUDE_TYPE",
  "TOR_OTHER_WFO",
...
calc_metrics <- function(prediction, actual) {
  ...
  ss_res <- sum((actual - prediction)^2)
  r_squared <- 1 - (ss_res / ss_total)
  
  return(list(MSE = mse, MAE = mae, R2 = r_squared))
}
...
The best lambda is 1.66, with the Mean Squared Error (mse) = 4,601,178, which told us the average difference between actual and prediction values. And the Mean Absolute Error (MAE) is 449.74. This means that, on average, the model's predictions differ from the actual outage values by about 450 units.
...
The best lambda is 111.74, with the Mean Squared Error (MSE) = 4,601,291, which tells us the average squared difference between actual and predicted values.
The Mean Absolute Error (MAE) is 447.36, meaning the model's predictions differ from the true outage values by about 447 units on average.
...
For the Random Forest model, the Mean Squared Error (MSE) is 3,222,250, which reflects the average squared difference between the predicted and actual values.
The Mean Absolute Error (MAE) is 307.04, meaning that, on average, the model's predictions are off by about 307 units from the actual outage values. This shows a noticeable improvement over the linear models in both accuracy and consistency.
...
Fore_res <- calc_metrics(pred, test_data$customers_day)
