...
Another key challenge involved the county variable, which contained hundreds of unique values. Incorporating such a high-cardinality categorical feature through one-hot encoding would have introduced excessive dimensionality and potentially degraded model performance. To avoid these issues, we removed county and instead retained the state variable, which preserved essential geographic variation in a more manageable form. The use of one-hot encoding for both state and EVENT_TYPE enabled the model to learn categorical effects without overwhelming the feature space.
